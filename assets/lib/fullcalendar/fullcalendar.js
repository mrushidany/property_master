...
// The second argument allows for an array of properties names who's object values will be merged together.
...
// Uses properties names as-is (no camel-case conversion). Will not make statements for null/undefined values.
...
var Grid = FC.Grid = Class.extend(ListenerMixin, {
	...
	// Must have a `grid` properties, a reference to this current grid. TODO: avoid this
	...
	*/
	...
	// Renders and assigns an `el` properties for each fill segment. Generic enough to work with different types.
...
Grid.mixin({
	...
	// Renders and assigns an `el` properties for each foreground event segment.
	// Only returns segments that successfully rendered.
...
var DayTableMixin = FC.DayTableMixin = {
	...
	// Computes and assigned the colCnt properties and updates any options that may be computed from it
...
TimeGrid.mixin({
	...
	// Given the name of a properties of `this` object, assumed to be an array of segments,
	// loops through each segment and removes from DOM. Will null-out the properties afterwards.
...
var View = FC.View = Class.extend(EmitterMixin, ListenerMixin, {
	...
	setRange: function(range) {
		$.extend(this, range); // assigns every properties to this object's member variables
		this.updateTitle();
	},
	...
	// Sets the view's title properties to the most updated computed value
...
function Calendar_constructor(element, overrides) {
	...
	// Returns a moment for the current date, as defined by the clients's computer or from the `now` option.
	...
	function initialRender() {
		...
		element.on('click.fc', 'a[data-goto]', function(ev) {
			...
			// properties like "navLinkDayClick". might be a string or a function
...
function EventManager() { // assumed to be a calendar
	...
	// Ensures the allDay properties exists and the timeliness of the start/end dates are consistent
...
Calendar.prototype.isSpanAllowed = function(span, constraint, overlap, event) {
	...
	for (i = 0; i < peerEvents.length; i++)  {
		...
		if (this.eventIntersectsRange(peerEvent, span)) {
			...
			// EventObject-specific or Source-specific `overlap` properties
			if (event) {
